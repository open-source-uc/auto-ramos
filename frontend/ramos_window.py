...
class RamosWindow(window_name, base_class):
    ...
    def init_gui(self):
        ...
        self.ramos_labels = {
            "1": [self.label_ramo_tit_1, self.label_ramo_1],
            "2": [self.label_ramo_tit_2, self.label_ramo_2],
            "3": [self.label_ramo_tit_3, self.label_ramo_3]
        }
        for nrc_layout in list(self.nrc_frames.values())[1:]:
            nrc_layout.hide()
        for tit, label in list(self.ramos_labels.values()):
            tit.hide()
            label.hide()
